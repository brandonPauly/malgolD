...
public class CodeGenerationVisitor implements ASTVisitor {
	...
	public void visit(FunctionDefinition d) {
		...
		for(Declaration dec : d.getParameters()){
			env.getCurrentScope().lookup(dec.getName()).setLocation(pos);
			pos += 4;
		}
		//fl = new FrameLayout();
		...
		result.append(generateInstruction("subl", "$" + env.getFrameSize(), "%esp"));
		result.append(buf);
		buf.setLength(0);
	}
	...
	public void visit(FunctionCallExpression e) {
		...
		if (argSize * 4 > env.getFrameLayout().currentOutgoingSize()){

...
public class SimplifyExpressionsVisitor implements ASTVisitor {
    ...
    private FunctionDefinition functionDefinitionResult = null;
    ...
    public void visit(FunctionDefinition f) {
        ...
        BlockStatement temp = null;
        assert (newStatements.size() == 1 && newStatements.get(0) instanceof BlockStatement) :
                "Body of function is not a Block during expression simplification.";

        //NOTE:  The body should be a single Block statement!!!
        temp = (BlockStatement) newStatements.get(0);
        functionDefinitionResult = new FunctionDefinition(f.getFirstToken(), f.getReturnType(), f.getName(), f.getParameters(), temp);
        newStatements = null;
    ...
    public void visit(FunctionCallExpression e) {
        LinkedList<Statement> tempS = new LinkedList<Statement>();
        LinkedList<Declaration> tempD = new LinkedList<Declaration>();
        ...
        for(Expression arg : e.getArguments()) {
            ...
            tempS.addAll(newStatements);
            tempD.addAll(newDeclarations);
            newArgs.add(expressionResult);
        }
        FunctionCallExpression newCall = new FunctionCallExpression(null, e.getName(), newArgs);
        VariableExpression newVar = VariableExpression.freshTemporary("FunctionCall");
        newVar.setType(e.getType());
        tempS.add(new AssignmentStatement(null, newVar, newCall));
        tempD.add(new Declaration(null, newVar.getName(), e.getType()));
        newStatements = tempS;
        newDeclarations = tempD;
    ...
    public void visit(ReturnStatement s) {
        s.getExpression().accept(this);
        Statement newReturn = new ReturnStatement(null, expressionResult);
